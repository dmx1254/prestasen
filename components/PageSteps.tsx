...
//   console.log(percentColor);
  ...
  const handleChangeStepFollowing = () => {
    setStepper((prevStepper) => prevStepper < steps.length - 1 ? prevStepper + 1 : prevStepper);
  };
  const handleChangeStepPrevious = () => {
    setStepper((prevStepper) =>
      prevStepper > 0 ? prevStepper - 1 : prevStepper
    );
  };
  useEffect(() => {
    ...
    <div className="relative flex flex-col items-center text-black w-full">
      <div className="flex flex-col items-center w-full">
        ...
        <div className="w-full">
          <Progress value={percentColor} className="w-full" />
        </div>
      </div>
      <div></div>

      {stepsJobs ? (
        <stepsJobs.stepComoponent
